...
import { resolveNetwork, getOrCreateWallet, formatWalletBackupNotice, recordContractDeployment } from './network';
...
const zkConfigPath = path.resolve(__dirname, '..', 'managed', 'offerstats');
...
const OfferStats = await import(pathToFileURL(contractPath).href);

// OfferStats has private witnesses (offerSecret, ctc) so withVacantWitnesses
// fails at Contract construction. Deploy only runs the constructor (no
// witness needed), so attach dummies. Real values flow later via
// callTx/findDeployedContract from the OfferStats tab.
const dummyWitnesses = {
  offerSecret: ({ privateState }: { privateState: unknown }) =>
    [privateState, new Uint8Array(32)] as [unknown, Uint8Array],
  ctc: () => [undefined as unknown, 750_000n] as [unknown, bigint],
};
...
async function createProviders(walletCtx: WalletContext) {
  ...
  return {
    privateStateProvider: levelPrivateStateProvider({
      privateStateStoreName: 'offerstats-state',
      accountId,
...
async function main() {
  console.log('\n╔══════════════════════════════════════════════════════════════╗');
  console.log(`║  Deploy OfferStats to ${network}`);
  console.log('╚══════════════════════════════════════════════════════════════╝\n');
